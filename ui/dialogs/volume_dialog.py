...
class VolumeDialog(QDialog):
    ...
    def __init__(self, mode="calc_volume", parent=None):
        super().__init__(parent)
        self.mode = mode
        self.setObjectName("VolumeDialog")
    ...
    def create_title_bar(self, parent_layout):
        ...
        title_map = {
            "calc_volume": "Lake Volume Calculation",
            "ev_curve": "E-V Curve Generation"
        }
        title_text = title_map.get(self.mode, "Volume Tool")
    ...
    def setup_dynamic_content(self):
        if self.mode == "calc_volume":
            self.widgets['ref_elev'] = self.create_input_row("Reference Elevation (m):", "1000", style="Gold", is_value=True)
            self.widgets['lake_dem'] = self.create_input_row("Lake DEM:", "Select .tif...", browse=True)
            self.widgets['lake_shp'] = self.create_input_row("Lake Polygon SHP:", "Select .shp...", browse=True)
            self.widgets['out_csv'] = self.create_input_row("Output CSV:", "Save as .csv...", browse=True, save_mode=True)

        elif self.mode == "ev_curve":
            self.widgets['lake_dem'] = self.create_input_row("Lake DEM Raster:", "Select .tif...", browse=True)

    def create_input_row(self, label_text, placeholder, browse=False, style="Green", save_mode=False, is_value=False):
        row_widget = QWidget()
        ...
        if browse:
            ...
            btn_browse.clicked.connect(lambda: self.browse_action(le, save_mode))
            row_layout.addWidget(btn_browse)
        
        self.form_layout.addWidget(row_widget)
        return le

    def browse_action(self, line_edit, save_mode):
        if save_mode:
